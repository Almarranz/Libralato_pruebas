...
# for i in range(1,4):    
    ...
    if len(index[0]>0):
        ...
        with open(pruebas+ 'pm_of_Ms_in_%s.txt'%(name), 'a') as f:
            f.write('\n'+ '%.7f %.7f %.7f %.7f %.7f %.7f %.7f %.7f %.0f'%(float(gal_coor[index[0],0]),float(gal_coor[index[0],1]),float(catal[index[0],4]),
                                                                          float(catal[index[0],6]),float(catal[index[0],0]),float(catal[index[0],1]),float(catal[index[0],2]),float(catal[index[0],3]),i))
