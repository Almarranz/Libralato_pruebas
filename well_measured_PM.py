...
paso=0.1
a=np.round(min(mag_ep12))
b=np.round(max(mag_ep12))
mag_b=np.digitize(mag_ep12, np.arange(a,b,paso), right=True)
...
fig, ax = plt.subplots(1,1, figsize=(10,10))
ax.scatter(mag,catal[:,5],s=0.1,color='k',alpha=1)
ax.scatter(mag_ep12[error_valid],pm_wmp[:,5],s=0.1,color='red',alpha=1)
ax.set_ylim(0,10)
ax.set_xlim(12,24)
